...
void
printwalk(int level, pagetable_t pagetable)
{
  if (level < 0 || level > 2)
    panic("printwalk");

  // there are 2^9 = 512 PTEs in a page table at any given level
  for (int i = 0; i < 512; i++) {
    pte_t pte = pagetable[i];
    if (pte & PTE_V) {
      uint64 pa = PTE2PA(pte);
      ...
      if ((pte & (PTE_R | PTE_W | PTE_X)) == 0) {
        printwalk(level + 1, (pagetable_t)pa);
      }
    }
  }
}
